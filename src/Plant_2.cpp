...
std::unique_ptr<ppgso::Mesh> Plant_2::mesh;
std::unique_ptr<ppgso::Texture> Plant_2::texture;
std::unique_ptr<ppgso::Shader> Plant_2::shader;
std::unique_ptr<ppgso::Shader> Plant_2::shadow_shader;


Plant_2::Plant_2(int x, int y, int z, std::string obj_name, std::string texture_name, float scale_factor) {
    scale *= scale_factor;
    rotation = {0,0,0};
    position.y = y;
    position.x = x;
...
bool Plant_2::update(float dt, SceneWindow &scene) {
    updateModelMatrix();
    return true;
}

void Plant_2::render(SceneWindow &scene) {
    shader->use();
...
void Plant_2::render_shadow(SceneWindow &scene, glm::mat4 lightSpaceMatrix) {
